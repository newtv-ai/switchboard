...
>
> **Status**: v1.3.1 · Last updated: 2026-07-27 · Core session lifecycle remediation + audit review
...
- [x] **Gate passed 2026-05-23**: `switchboard run claude` registers a wrapped session, both phone and desktop browsers see it in the session list, both can attach and view live TUI, bidirectional input sync confirmed (phone types → desktop sees it). Multi-client PTY sizing (min cols + min rows) negotiates a workable shared dimension; quick-actions toolbar hidden on desktop where physical keys exist.
...
- **Per-client size tracking with split policy (MIN cols, MIN rows)**: `Session.attach()` now returns a `ClientHandle` and tracks each attached client's reported viewport. `refitToClients()` picks `min(cols)` and `min(rows)` across all attached clients. Rationale: in alt-screen TUIs (claude code, codex), cursor positions ABOVE a small client's row count are clipped and content there literally never enters that client's xterm buffer — so MIN rows is necessary for the input prompt to be reachable on small clients. Cols mismatches clip the right side but the left (where prompts live) renders, so MAX cols keeps wider clients usable.
...
- 2026-07-27 — v1.3.1 — **Session-list repaint fix**: the v1.2 activity broadcast is removed. It emitted a throttled event per printing Session, and because every list subscriber answers an event with a full snapshot, a browser parked on the session list re-rendered once per second for as long as any CLI produced output (measured: 1.05 snapshots/s). List snapshots are now driven purely by lifecycle events.
